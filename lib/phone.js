...
/*
 * Try to find country id by number (or its part).
 * return sample: { id: 'US', phone: '18773975248', code: '1', line: '8773975248',
 *   type: 'tollFree', format: '+1 (877) 397-5248' }
 */
Phone.prototype.countryFromNumber = function(phone, smooth) {
	...
	_.each(guess, function(item) {
		...
		_.each(item.types, function(v, k) {
			var found = v.expr.test(
				smooth ? line + v.example.substring(test.length) : line
			);
			if(!found) return;
			var result = {
				id: item.id,
	...
	if(guess2.length <= 1) return this._format(guess2[0]);
